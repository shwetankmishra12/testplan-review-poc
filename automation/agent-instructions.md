...
Paste **everything below the line** into Cursor Automation → **Agent Instructions**.

Standards checklist:
https://confluence.cohesity.com/pages/viewpage.action?pageId=1313507592

Worker must be started from this repo root so `$PWD` points here.
...
# Env already set on worker: CONFLUENCE_URL, CONFLUENCE_PERSONAL_ACCESS_TOKEN
# Fallback: ~/.confluence_pat
```
...
```
If some comments fail, retry those with a more unique `anchor` from the page. Do not fall back to footer comments unless the user asks.
